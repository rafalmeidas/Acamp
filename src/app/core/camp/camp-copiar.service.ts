...
export class CampCopiarService {
    ...
    listByUser(page = 1, limit = 6) {
        return this.http.get<CampPaginate>(`${API_URL}user/camp?page=${page}&limit=${limit}&order=-created_at`).pipe(map(dados => dados));
    }
    ...
    loadByID(id) {
        return this.http.get<Camp>(`${API_URL}user/camp/${id}`);
      }
